...
If you're new to DFFML. See the :doc:`/installation` and :doc:`/quickstart`
documents to get started, then come back here to try out the new features.
...
- :doc:`Natural Language Processing (NLP) Models and Operations </tutorials/dataflows/nlp>`
  ...
  - Himanshu, one of our GSoC 2020 students, implemented Natural Language
    ...
    :ref:`dffml-operations-nlp <plugin_model_dffml_operations_nlp>` on the
...
- Remote execution
  ...
  - The :doc:`HTTP service <plugins/service/http/index>` already allows users to
    access all the DFFML command line and Python APIs over HTTP. We are going to
    integrate the :doc:`/api/high_level` API with the HTTP service. A remote
